...
@section('page_title', 'Edit Station - SGR')
...
<div class="max-w-2xl mx-auto space-y-4">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-gray-900">Edit Station</h1>
            <p class="text-sm text-gray-500">{{ $station->name }}</p>
        </div>
    </div>

    <form action="{{ route('stations.update', $station) }}" method="POST" class="bg-white rounded-xl border p-6 space-y-5">
        @csrf @method('PUT')
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Parking Lot <span class="text-danger-500">*</span></label>
                <select name="parking_lot_id" required class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-maroon-500 focus:ring-2 focus:ring-maroon-200 outline-none transition-all text-sm">
                    <option value="">Select a lot</option>
                    @foreach($lots as $lot)
                    <option value="{{ $lot->id }}" {{ old('parking_lot_id', $station->parking_lot_id) == $lot->id ? 'selected' : '' }}>{{ $lot->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <select name="supervisor_id" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-maroon-500 focus:ring-2 focus:ring-maroon-200 outline-none transition-all text-sm">
                    ...
                    <option value="{{ $sup->id }}" {{ old('supervisor_id', $station->supervisor_id) == $sup->id ? 'selected' : '' }}>{{ $sup->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Name <span class="text-danger-500">*</span></label>
                <input type="text" name="name" value="{{ old('name', $station->name) }}" required class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-maroon-500 focus:ring-2 focus:ring-maroon-200 outline-none transition-all text-sm">
            </div>
            <div>
            ...
            <div class="sm:col-span-2">
                ...
                <textarea name="notes" rows="3" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-maroon-500 focus:ring-2 focus:ring-maroon-200 outline-none transition-all text-sm">{{ old('notes', $station->notes) }}</textarea>
            </div>
        </div>
        <div class="flex items-center justify-end gap-3 pt-2">
            ...
            <button type="submit" class="px-4 py-2 text-sm font-bold text-white bg-maroon-500 rounded-lg hover:bg-maroon-600 transition-colors">Update Station</button>
        </div>
    </form>
</div>
@endsection
