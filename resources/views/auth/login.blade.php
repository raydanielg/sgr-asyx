...
<div class="w-full max-w-md" style="animation: simpleFadeIn 0.4s ease-out both;">
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        ...
        <div class="bg-gradient-to-br from-maroon-600 to-maroon-700 px-8 py-8 text-center">
            ...
            <p class="text-maroon-100 text-sm mt-1">Sign in to your SGR account</p>
        </div>
                ...
                <div class="flex items-center justify-between">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="remember" id="remember" {{ old('remember') ? 'checked' : '' }}
                            class="w-4 h-4 rounded border-gray-300 text-maroon-600 focus:ring-maroon-500">
                        <span class="text-sm text-gray-600">Remember me</span>
                    </label>
                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}" class="text-sm font-medium text-maroon-600 hover:text-maroon-700 transition-colors">Forgot password?</a>
                    @endif
                </div>
